...
```{r}
model.TrainLM=lm(HousePriceOfUnitArea ~ TransactionDate + HouseAge + DistanceToMRT + NumberOfConStores + Latitude, data=Train)
...
#The Cook's Distance plot shows that 271 has a pretty high value of about 0.26 but it is not more than 0.5 so it is not influential.
```
...
```{r}
ln.TrainLM=lm(log(HousePriceOfUnitArea) ~ TransactionDate + HouseAge + DistanceToMRT + NumberOfConStores + Latitude, data=Train)
